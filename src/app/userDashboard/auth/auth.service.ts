...
export class AuthService {
  ...
  async login(loginAuthDto: LoginAuthDto) {
    ...
    // 🌟 الخطوة الأساسية: تحويل البريد الإلكتروني إلى أحرف صغيرة قبل البحث
    const lowercasedEmail = email.toLowerCase(); 

    const user = await this.userModel.findOne({
      email: lowercasedEmail, // استخدم البريد الإلكتروني المحول هنا
    });

    if (!user) {
      // سيتم تنفيذ هذا إذا لم يتم العثور على المستخدم بعد التوحيد
    ...
    const payload = { email: user.email, ـid: user._id };
